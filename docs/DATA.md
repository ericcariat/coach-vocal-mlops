...
| `oww_models` | Front-ends **openWakeWord** v0.5.1 (<https://github.com/dscripka/openWakeWord>) : `melspectrogram.onnx` + `embedding_model.onnx` (embeddings Google speech), téléchargés le 2026-08-13 | Aucune — servent à l'adaptateur du banc (`bench --run *.onnx`) pour évaluer des têtes oWW avec LEUR front-end et NOTRE machine à états | Apache 2.0 (code) ; embeddings Google Apache 2.0 |
| `summre_train` | **SUMM-RE split train** — 2 pistes des réunions `001a_PARL`, `001b_PADL` (39,8 min), **disjointes** du banc (004c/006b/015b) et de val_ambient (017a/018a) | 16 kHz mono PCM16 ; zéro occurrence du mot (vérifié sur les alignements) ; négatifs d'ENTRAÎNEMENT via la source `continuous_dir` | CC BY-SA 4.0 |
| `val_ambient` | **SUMM-RE** — 2 pistes de réunions (`017a_EBRZ_054`, `018a_EARZ_055`, 34,5 min) **disjointes des 3 réunions du banc** | 16 kHz mono PCM16 ; zéro occurrence du mot (vérifié sur les alignements) ; sert à l'élection `fa_ambient` des candidats (jamais au banc) | CC BY-SA 4.0 |
| `rir_mit` | **MIT IR Survey** (<https://mcdermottlab.mit.edu/Reverb/IR_Survey.html>), téléchargé le 2026-08-13 : 270 réponses impulsionnelles de lieux réels | 32 kHz → 16 kHz mono (`16k/`), énergie normalisée au chargement ; servent à l'augmentation par réverbération (`augmentation.rir_prob`) | Libre pour la recherche (citation demandée) |
